...
For formulas, please note the use of `as_function` function from package **rlang** to retrieve the user input. As a leverage of tidyverse, the **(.)** argument can be used in anonymous functions or named one, prefixing the function by a **~**.
...
These formulas use the `if/else` statements to permit multiple tests upon the type of variable. They can use functions such as `is.factor`, `is.numeric`, and `is.logical`.  
...
`~if(is.factor(.)) percent(.) else if (is.normal(.)) mean(.))`
...
iris %>%
  desctable(stats = list("N"      = length,
                         "%/Mean" = ~if (is.factor(.)) percent(.)
                         else if (is.normal(.)) mean(.),
                         "Median" = ~if(!is.normal(.)) median(.))) %>%
  datatable()
```
<br>
...
The named list can contain named functions such as `t.test` prefixed with *~* but also pipelines using *pipe* character from **magrittr** and **(.)** argument. Please note, the necessity to use **~** before any function, due to use of **rlang** `as_function`  function.
...
iris %>%
  group_by(Species) %>%
  desctable(tests = list(.auto = tests_auto,
                         Sepal.Width = ~oneway.test(., var.equal = F),
