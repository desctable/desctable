...
Enter *desctable*, a modern package for descriptive and comparative tables generation, integrated with the *tidyverse* set of tools, completely customizable yet with reasonable defaults, with an agnostic tidy output that can be further modified using standard R tools.
...
```
<br>
You need to load these two packages first if you want to use them.  
...
When displayed as a flat dataframe, the group headers don't appear. You can see them by inspecting the resulting object, which is a list of dataframes, each named after the grouping factor and its levels (with sample size for each).
